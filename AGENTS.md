...
- Claude Code hook 已配置在 `.claude/settings.local.json`，会在工具使用后和停止时执行 `scripts/update-current.sh`；如果 hook 失败，手动执行。
...
- 对外交付的 PRD/评审/开发报告优先使用 HTML 格式；Markdown 可保留为源稿，但 HTML 为主交付件，便于浏览器直接查看和评审
